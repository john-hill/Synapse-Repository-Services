...
/**
 * A conversational entity metadata specialist agent. Each instance maintains its own
 * chat memory and is intended for a single task delegation (multi-turn within that task,
 * but discarded after).
 */
public class EntityMetadataSpecialist implements Agent {

	private final ChatClient chatClient;
	private final String conversationId;

	EntityMetadataSpecialist(ChatModel chatModel, StackConfiguration stackConfig,
		...
		this.chatClient = ChatClient.builder(chatModel)
				...
				.defaultToolCallbacks(entityMetadataSpecialistTools.getToolCallbacks())
